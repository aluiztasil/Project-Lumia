...
def home():
    ...
    file_path = '/Users/andretavares/Documents/GitHub/Project-Lumia-v2/Curve_clasifier_client/data/database_TL_other.json'
    extension = '.json'
    # if extension == '.csv':
    ...
    # session_size = max(0, min(len(data.keys()), int(session['session_size'])))
    sampled_curve_names = random.sample(list(data.keys()), int(session['session_size'])) #session_size)

 
    curve_name = sampled_curve_names[len(user_responses)]
    # Extract metadata
    metadata = data[curve_name]['curve_metadata']
    curve_data = data[curve_name]['curve_data']
    # Extract values
    ...
    plt.plot(temperature, curve_data)
    plt.xlabel('Temperature')
    plt.ylabel('Luminescent')
    plt.title('Temperature vs Luminescent')
    plt.grid(True)
    plt.show()
    plt.savefig('/Users/andretavares/Documents/GitHub/Project-Lumia-v2/Curve_clasifier_client/static/image.png', format='png') # --  up to this part itworks, the page doesnt render

    #Calculate session size
    countdown = 'Current Session: ' + str(len(user_responses)) + ' / ' +str(session['session_size'])
    ...
    if request.method == 'POST':
        ...
        if len(user_responses) % int(session['session_size'])==0:  # save file only in the end of the session (?)
            try:
            ...
            df_out.to_csv("Curve_clasifier_client/data/output_{}.csv".format(session['username']), index=False)
    ...
    return render_template('index.html', curve_id=curve_name, statement=statement,
                           ...
                           rate_value=rate_value, an_temp_value=an_temp_value, an_time_value=an_time_value, countdown=countdown)
